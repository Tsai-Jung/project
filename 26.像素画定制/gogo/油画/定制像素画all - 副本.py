...
XX = int(input("宽像素块"))
YY = int(input("长像素块"))
morecolor = int(input("几种颜色"))
checkcolor = input("是否要统计颜色数量y/n")

# 使用示例：将图片尺寸改为800x600并保存为新文件
resize_image(me, 'me2.png', (XX*160,YY*160))

# 使用函数转换图像并保存结果
reduce_colors('me2.png', 'me3.png', num_colors=morecolor)
...
for m in range(10):
    for k in range(150):
        for l in range(100):
            try:
                image.putpixel((150*m+k, l), num_list[m])
                image.putpixel((150*m+k, l+100), num_list[m+10])
                image.putpixel((150*m+k, l+200), num_list[m+20])
                image.putpixel((100*m+k, l+200), num_list[m+30])
            except:
                pass
    ...
    text = str(listnum[m])
    draw.text((150*m+28, 20), text, font=font, fill='red')
    text = str(listnum[m+10])
    draw.text((150*m+28, 120), text, font=font, fill='red')
    text = str(listnum[m+20])
    draw.text((150*m+28, 220), text, font=font, fill='red')
