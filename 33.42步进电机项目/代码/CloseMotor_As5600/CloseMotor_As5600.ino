...
uint8_t broadcastAddress[2][6] = {
  ...
  { 0x4C, 0x75, 0x25, 0x08, 0x15, 0xB4 },  //电脑接收器
...
void mysetup0() {
  ...
  //初始化as5600
  as5600.begin(4, 5);                      //  前面4(SDA) 后面5(SCL)
  as5600.setDirection(AS5600_CLOCK_WISE);  // default, just be explicit.
  int b = as5600.isConnected();
  Serial.print("Connect: ");
  Serial.println(b);
  delay(100);
  if (as5600.readAngle() < 3000 and as5600.readAngle() > 1000) {
    digitalWrite(dirPin, LOW);
  } else digitalWrite(dirPin, HIGH);

  for (int x = 0; x < (STEPS_PER_REV * 2); x++) {
    if (as5600.readAngle() < 30) break;
...
void myloop0() {  //42步进电机
  ...
  if (typeword == 1) {
    ...
    pernum = numdata;
    for (int x = 0; x < (STEPS_PER_REV * 2); x++) {
      if (abs(as5600.readAngle() - numdata) < 30) break;
      digitalWrite(stepPin, HIGH);
      delayMicroseconds(1000);
      digitalWrite(stepPin, LOW);
      delayMicroseconds(1000);
  ...
  for (int x = 0; x < (STEPS_PER_REV * 2); x++) {
    if ((abs(as5600.readAngle() - pernum) < 30) or (abs(4070 - as5600.readAngle() + pernum)) < 30) break;
    digitalWrite(stepPin, HIGH);
    delayMicroseconds(1000);
    digitalWrite(stepPin, LOW);
